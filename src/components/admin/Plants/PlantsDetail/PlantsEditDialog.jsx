...
export default function PlantsEditDialog({
    ...
    if (imageFile) {
      uploadFileAndReturnUrl(`/public/plants/${uuidv4()}.jpg`, imageFile)
        .then((url) => {
          ...
          return aupdate(`/plants/${item._id}`, updatePlantData);
        })
        .then(() => {
      ...
      aupdate(`/plants/${item._id}`, updatePlantData)
        .then(() => {
          showSuccessToast("Plant updated successfully!");
          onFinish();
          onClose();
  ...
  const handleError = (err) => {
    ...
    if (status === 403) {
      ...
      showErrorToast("Error updating plant.");
    }
  };
  ...
  const selectedGenus = useMemo(() => {
    return genusList.find((genus) => genus._id === formData.genus_id);
  }, [genusList, formData]);

  const selectedPlantType = useMemo(() => {
    return plantTypeList.find(
      (plantType) => plantType._id === formData.plant_type_id
    );
  }, [plantTypeList, formData]);
