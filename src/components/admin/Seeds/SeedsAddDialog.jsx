...
export default function SeedsAddDialog({
  open,
  onClose,
  onFinish,
  fields = plantFields,
  ...
  const handleSubmit = () => {
    ...
    uploadFileAndReturnUrl(`/public/seeds/${uuidv4()}.jpg`, imageFile)
      .then((url) => {
        ...
        return apost('/seeds', finalForm);
      })
      .then(() => {
        showSuccessToast("Seed added successfully!");
        onFinish()
        onClose();
      })
      .catch((err) => {
  ...
  return (
    <DialogBasic
      className="seeds-add-dialog"
      title="Adding new seed"
      open={open}
      onClose={onClose}
      footer={
        <DialogActions>
